...
-   **Centralized IP Management:** Automated the allocation of IP addresses and critical network settings from a single server.
-   **Active Directory Integration:** Ensured clients automatically receive the correct DNS server setting (pointing to the Domain Controller) to maintain seamless domain functionality.
-   **Operational Verification:** Confirmed the server successfully authorizes and leases addresses, and clients correctly receive their configuration.
